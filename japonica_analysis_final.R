# Edu PGS analysis in J-MICC (JAPONICA)
...
output_dir <- "./output/"
load("dataset_japonica_imp.RData")
...
gtsummary::as_hux_xlsx(Table1, file = paste0(output_dir, "jap_00_table1.xlsx"))
...
all |>
  cbind(sex) |>
  write_csv(paste0(output_dir,"jap_01_edupgs_univ.csv"))
...
ggsave(paste0(output_dir, "jap_fig1A.png"), width = 10, height = 8, dpi = 600)
...
ggsave(paste0(output_dir, "jap_fig1B.png"), width = 9, height = 6, dpi = 600)
...
for (trait in traits) {
  ...
  formula1 <- as.formula(
    paste(trait, "~ prs_edu + sex + age + I(age^2) + adi2010_2 +
          factor(birth_year) + PC1 + PC2 + PC3 + PC4 + PC5")
  )
...
write_csv(final_results, paste0(output_dir,"jap_02_edupgs_lifestyle.csv"))
...
write_csv(
  ...
  paste0(output_dir, "jap_12_edupgs_interaction.csv")
)
...
write_csv(final_results, paste0(output_dir,"jap_03_edupgs_phenotype.csv"))
...
write_csv(
  interaction_results_df,
  paste0(output_dir, "jap_13_edupgs_interaction.csv")
)
...
write.csv(table, paste0(output_dir, "jap_04_edupgs_mortality.csv"))
...
write.csv(final_results, paste0(output_dir, "jap_05_edupgs_poisson.csv"))
...
write.csv(
  interaction_results_df,
  paste0(output_dir, "jap_14_edupgs_interaction.csv"),
  row.names = FALSE
)
...
for (trait in traits) {
  ...
  formula <- as.formula(paste(trait, "~ prs_edu_edu + sex + age + I(age^2) + adi2010_2 + factor(birth_year) + PC1 + PC2 + PC3 + PC4 + PC5"))
  
  model <- rqlm(formula, data = dt_tmp, family = "gaussian", digits = 100)
...
write_csv(final_results, paste0(output_dir,"jap_06_edupgs_edu_lifestyle.csv"))
...
write_csv(final_results, paste0(output_dir,"jap_07_edupgs_edu_phenotype.csv"))
...
write_csv(final_results, paste0(output_dir,"jap_08_edupgs_edu_mortality.csv"))
...
# Univ Comp: 25.0%
dt <- dt |>
  mutate(prs_edu_binary = cut(prs_edu, breaks = c(-Inf, quantile(prs_edu, 0.75), Inf), labels = c(0, 1)))
...
for (trait in traits) {
  ...
  formula1 <- as.formula(paste(trait, "~ univ_comp + sex + age + I(age^2) + adi2010_2 + factor(birth_year) + PC1 + PC2 + PC3 + PC4 + PC5"))
  formula2 <- as.formula(paste(trait, "~ prs_edu_binary + sex + age + I(age^2) + adi2010_2 + factor(birth_year) + PC1 + PC2 + PC3 + PC4 + PC5"))
...
write_csv(final_results, paste0(output_dir,"jap_09_edu_lifestyle.csv"))
...
write_csv(final_results, paste0(output_dir,"jap_10_edu_phenotype.csv"))
...
write.csv(final_results, paste0(output_dir, "jap_11_edu_poisson.csv"))